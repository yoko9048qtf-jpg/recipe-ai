...
- `rewrites`:
  1. `/api/(.*)` → `api/index.js`（サーバーレス関数）にすべて転送
  2. `/(.*)` → `/index.html`（2026-07-05追加。`/privacy`等のポリシーページへ直接アクセス・リロードしても
     SPAとして正しく表示するためのフォールバック。実ファイルが存在する静的アセットは、この rewrite より
     先にファイルシステム側で解決されるため影響しない）
